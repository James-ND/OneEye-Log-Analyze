...
def main(csv_file, count, stat):
    df = pd.read_csv(csv_file, 
                     ...
                     dtype={
                        ...
                        'duration': np.float64,
                        'view': np.float64,
                        'thrift_runtime': np.float64,
                        'db': np.float64
                     }
                )
    total_count = len(df)
    df_grp = df.groupby('case')
    url_info = {}
    url_usage = {}
    for n, g in df_grp:
        url_info[n] = g
    ...
    if stat.lower() == 'duration':
        ...
        for i in range(len(top_index)-1, -1, -1):
            url = top_df['case'].get(top_index[i])
    ...
    header = "url,count,usage,duration_min, duration_max, duration_mean,duration_std\n"
    fmt = "{url},{count},{usage},{duration_min},{duration_max},{duration_mean},{duration_std}\n"
